    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            case OBJ_BOUND_METHOD: {
                ...
                // -argCount skips past the args and
                // the -1 adjusts for the fact that stackTop points just past the last used stack slot.
...
static bool invoke(ObjString* name, int argCount) {
    ...
    // deal return a field as method call.
    Value value;
